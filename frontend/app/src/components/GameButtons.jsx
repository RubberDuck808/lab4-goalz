...
export default function GameButtons({ children, onClick, className = '', variant = 'task', size = 'default', ...props }) {
  ...
  // Mapped exact user styles: display flex, h:48, p: 0 0.5 4 0.5, rounded 13px, text white
  const baseStyle = `font-custom-button flex items-center justify-center ${sizes[size] || sizes.default} h-[48px] pt-0 pr-[0.5px] pb-[4px] pl-[0.5px] rounded-[13px] font-bold transition-all tracking-wide uppercase text-white`;
