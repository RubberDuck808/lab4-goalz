...
export default function Sensor({ formData, setFormData, onSubmit }) {
    ...
    <div className='flex flex-col gap-3 h-full w-full'>
        ...
        <div className='grow-1 flex flex gap-3 py-0 px-4'>
            <div className='flex-col items-center justify-between gap-3 grow-1'>
              <div className='mt-2'>
                <p className='font text-sm text-gray-500'>Sensor id</p>
                <input 
                  type="text" value={formData.sensorId} 
                  onChange={(e) => setFormData({...formData, sensorId: e.target.value})} 
                  className='w-full h-6 border border-gray-300 rounded px-2 text-sm' 
                />
              </div>
              <div className='mt-2'>
                <p className='font text-sm text-gray-500'>Sensor type</p>
                <input 
                  type="text" 
